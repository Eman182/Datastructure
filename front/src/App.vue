...
export default {
   ...
   ConcatForPercent :function(){
    var str=String(this.text1);
      ...
      if(str.charAt(str.length-1)==')'){
        ...
        str=str.substr(0,i+1)+'(('+str.substr(i+1,str.length)+")/100)"
        this.text1=str;
      }else{
        //it must be number or . if the format is true  then we want to reach the character at which we will put
        //the paranthess
        ...
        if(str.charAt(i)=='.'||(str.charAt(i)<='9'&&str.charAt(i)>='0')){
          while ((str.charAt(i)>='0'&&str.charAt(i)<='9')||(str.charAt(i)=='.')){
           ...
           //now we reached the character we want to put paranthess after it 
          
            str=str.substr(0,i+1)+'(('+str.substr(i+1,str.length)+')/100)';
            this.text1=str;
           
        } else{
          //it is wrong format the backend will handle it so only put the paranthess before and after
   ...
   ConcatForSquareRoot:function(){
    if( String(this.text1).length<34){
      ...
      if(str.charAt(str.length-1)==')'){
        ...
        str=str.substr(0,i+1)+'(('+str.substr(i+1,str.length)+")^0.5)"
        this.text1=str;
      }else{
        //it must be number or . if the format is true  then we want to reach the character at which we will put
        //the paranthess
   ...
   ConcatForSquaring :function(){
    if(String(this.text1).length<35){
      ...
      if(str.charAt(str.length-1)==')'){
        ...
        str=str.substr(0,i+1)+'(('+str.substr(i+1,str.length)+")^2)"
        this.text1=str;
      }else{
        //it must be number or . if the format is true  then we want to reach the character at which we will put
        //the paranthess
  ...
  calc : function(){//get the result
      ...
      if(st.length==0){
        this.text1="";
      }else if((st.length==1&&st.charAt(0)=='.')){
        this.text1="0";
      }
      else{
